...
#pragma omp atomic
    ...
    char votefile[20];
    strcpy(votefile, "vote/vote");
    strcat(votefile, argv[1]);
    strcat(votefile, ".txt");

    std::ofstream output(votefile);

    
    for (int l = 0; l < 2 * maxDistance + 1; l++)
    {
        for (int k = 0; k < nangle; k++)
        {
            output << votes[l * nangle + k] << ","; // behaves like cout - cout is also a stream
        }
        output << "\n" << endl;
    }
